...
- Runtime executor: `tools/tool_mapping_runtime.py`
- Instrukcja kernela runtime (prompt/system): `docs/runtime_kernel_instruction.md`
- Krótkie instrukcje do pola **Instructions** w Custom GPT: `docs/custom_gpt_instructions.txt`
...
## 7) Runtime tool_mapping layer (wykonawcza)

Plik: `tools/tool_mapping_runtime.py`

Zapewnia brakującą warstwę wykonawczą:
- routing `AUTO_PIPELINE` / `DASHBOARD` na podstawie inputu,
- wykonanie kroków kontraktu sekwencyjnie (bez skip/reorder),
- mapowanie akcji kontraktu na realne metody `execution_tool` przez `tool_mapping`,
- obsługę operacji wewnętrznych (decode/encode/validate/hash/merge/fallback/dashboard),
- końcowe składanie odpowiedzi wg `output.returns`.

Minimalne użycie:
- utwórz adapter `execution_tool` (metody z OpenAPI operationId),
- opcjonalnie `GPTExecutionHelper` dla renderera UI,
- uruchom `ToolMappingRuntime(...).execute(input_data)`.

## 8) Zasady bezpieczeństwa i spójności
